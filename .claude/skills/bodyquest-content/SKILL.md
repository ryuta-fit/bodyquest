...
## モードA: 既存分野を増量する
1. 現在数を確認: `node .claude/skills/bodyquest-content/scripts/validate.js`
...
5. `node .claude/skills/bodyquest-content/scripts/dedup.js`（完全重複設問を除去）。
6. `node .claude/skills/bodyquest-content/scripts/validate.js`（idDup/schemaBad/over1000/stemDup=0 を確認）。
...
## ブラウザ実機確認（重要・キャッシュ注意）
- プレビューは `mcp__Claude_Preview__*`（port は `.claude/launch.json` 参照）。`preview_start`→`preview_resize` mobile。
